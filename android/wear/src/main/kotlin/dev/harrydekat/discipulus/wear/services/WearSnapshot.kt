...
object WearSnapshot {
    ...
    fun showCancelledLessons(context: Context): Boolean = context
        .getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        .getBoolean("show_cancelled_lessons", false)

    fun schedule(context: Context): List<ScheduleEvent> {
        val includeCancelled = showCancelledLessons(context)
        return read<Map<String, List<ScheduleEvent>>>(context, "cached_schedule")
            ?.values
            ?.flatten()
            ?.filter { includeCancelled || !it.isCanceled }
            ?.sortedBy(ScheduleEvent::startTime)
            ?: emptyList()
    }
