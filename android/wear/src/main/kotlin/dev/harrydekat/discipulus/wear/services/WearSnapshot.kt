...
object WearSnapshot {
    ...
    fun schedule(context: Context): List<ScheduleEvent> =
        read<Map<String, List<ScheduleEvent>>>(context, "cached_schedule")
            ?.values?.flatten()?.sortedBy(ScheduleEvent::startTime) ?: emptyList()
    ...
    fun nextEvent(context: Context): ScheduleEvent? {
        ...
        return events.firstOrNull { it.startTime.after(now) && !(it.isCompleted && it.infoType == 1) }
    }
